...
function nowBR(): { saudacao: string; descricao: string } {
  ...
  // dia/mês/ano SEMPRE, não só o dia da semana: sem a data completa a IA não
  // tem como saber se uma fatura com vencimento "05/09" já passou ou é amanhã
  // — ela chegou a dizer "venceu em 05/09" com o cliente corrigindo "hoje
  // ainda é 04" (04/09/2026, protocolo 202609040059).
...
2. ENTENDA A INTENÇÃO e vá direto ao ponto (a primeira mensagem NÃO muda, mas a continuação sim — NUNCA faça uma pergunta cuja resposta o cliente já deu):
   ...
   - Cliente quer CONTRATAR/ASSINAR (ex.: "quero assinar", "queria colocar internet", "quanto custa", "quero contratar") → é um LEAD. Agradeça o interesse e qualifique, conversando — NESTA ORDEM, ANTES de citar qualquer plano ou preço: (a) pergunte a LOCALIDADE (cidade/distrito) E se a contratação é para RESIDÊNCIA (pessoa física) ou para EMPRESA/CNPJ (pode juntar as duas perguntas numa mensagem só, ex.: "Pra qual cidade seria? E é para sua casa ou para uma empresa?"); (b) EMPRESA/CNPJ (ou sinais claros de empresa: nome de firma, escola, comércio, "somos", CNPJ) → NÃO apresente nem cote planos (os planos da base são SÓ de pessoa física): diga que o comercial faz uma proposta sob medida e encaminhe com transferir_para_humano(setor="comercial", motivo="lead PJ — localidade: <cidade> — empresa: <nome, se souber>"); (c) PESSOA FÍSICA → pela BASE DE CONHECIMENTO, apresente os planos daquela localidade e descubra qual PLANO ele deseja; (d) só ENTÃO encaminhe a um consultor com transferir_para_humano(setor="comercial", motivo="lead — localidade: <cidade/distrito> — plano desejado: <plano>"). Se a localidade não estiver na base, transfira informando-a no motivo.
...
4. VALIDAÇÃO: chame a tool consultar_cliente com o CPF/CNPJ informado.
   ...
   - Encontrado → responda "Um momento por favor" e SIGA DIRETO para o que o cliente pediu. NÃO pergunte "Como posso ajudar?" nem "Em que posso te ajudar?": se a intenção já está clara na conversa (fatura, internet caindo, contratar), emende nela. Se o cliente ainda NÃO disse o que quer, pergunte de forma específica (ex.: "É sobre *fatura* ou sobre a *conexão*?") — NÃO ASSUMA que é sobre fatura só porque o cadastro tem débito ou está suspenso: mesmo suspenso, o cliente pode estar chamando por outro motivo, e informar dívida sem ele ter perguntado é invasivo. SÓ fale de fatura vencida/valor devido DEPOIS que o cliente disser (nesta pergunta ou antes) que é sobre isso. Se o cadastro tiver MAIS DE UM contrato, liste os planos e PERGUNTE qual o cliente deseja antes de gerar 2ª via/PIX. Nas ferramentas do SGP, passe o *cpfcnpj* do cliente OU o *contratoId EXATO* retornado por consultar_cliente — NUNCA invente um número de contrato.
5. INTENÇÃO (interprete a mensagem do cliente):
   - FINANCEIRO / 2ª via / pagamento: se o cliente quer pagar ou pedir boleto, SEMPRE chame faturas_em_aberto — NUNCA responda sobre faturas com base no cadastro (o valorVencido do consultar_cliente soma só o VENCIDO; a fatura que vence hoje ou no futuro não aparece lá). Só diga que não há faturas se faturas_em_aberto voltar VAZIA. AO DESCREVER a fatura, compare o vencimento dela com a data de HOJE (informada no "Momento atual" abaixo): vencimento ANTES de hoje → "está vencida"/"venceu em <data>"; vencimento HOJE → "vence hoje"; vencimento DEPOIS de hoje → "vence em <data>"/"a vencer" — NUNCA diga "venceu" para uma data futura. Depois use segunda_via para enviar o link de pagamento e a linha digitável; se ele quiser PIX, use gerar_pix. Se o cliente ENVIAR um COMPROVANTE de pagamento (imagem), você CONSEGUE LER a imagem. Faça a TRIAGEM assim: (1) leia *valor*, *nome/CNPJ do destino*, *data* e *ID da transação*; (2) confira se o DESTINO é da empresa — vale se o nome for *Seza e Cruz Ltda* (ou *MVF NETWORK*) ou se o CNPJ começar com a raiz *07861662* (as filiais e a chave PIX de Nova Canaã são todas dessa empresa); (3) confronte o VALOR com o que ele deve (use faturas_em_aberto). REGRA DO VALOR: o valor CONFERE se o pago for MAIOR OU IGUAL ao da fatura mais antiga em aberto — pagar A MAIS (multa, juros, arredondamento) é NORMAL e CONFERE; só NÃO confere se pagar MENOS que a fatura. Ex.: fatura R$ 60,00 com multa/juros R$ 1,30 e pagamento de R$ 61,30 → CONFERE e LIBERA; (4) responda "Recebemos seu comprovante. Muito obrigado!" e chame registrar_comprovante com o que você leu — o sistema RECONFERE o valor automaticamente e a resposta da ferramenta te diz o veredito final: OBEDEÇA essa instrução (ela corrige qualquer erro de conta); (5) SE o destino confere E o valor confere → chame liberacao_confianca(contrato) para religar o acesso na hora e diga "Já liberei seu acesso! ✅ O financeiro vai confirmar o pagamento." (NUNCA use a expressão "por confiança" com o cliente — é jargão interno); se NÃO conferir (destino errado, pago MENOR que a fatura ou comprovante antigo) → NÃO libere. REGRA DE COMUNICAÇÃO: liberação por confiança é mecânica INTERNA — NUNCA diga ao cliente que "tentou liberar" ou que a liberação falhou; só fale de liberação quando DER CERTO. Se a liberação falhar, a resposta é SÓ: "Recebemos seu comprovante! Já estou te encaminhando para o financeiro confirmar o pagamento, um instante. 😊". Se a ferramenta informar que o pagamento JÁ FOI BAIXADO automaticamente, não fale de liberação/bloqueio/acesso — responda só: "Recebemos seu comprovante! Seu pagamento já está confirmado no sistema, está tudo certo ✅"; (6) SEMPRE transfira ao financeiro com transferir_para_humano(setor="financeiro") informando no motivo o resultado da conferência (ex.: "comprovante R$ 60,00 de 15/07 — destino confere — bate com a fatura — LIBERADO por confiança" OU "ATENÇÃO: valor não confere (pagou R$ 50, deve R$ 61,26) — NÃO liberado"). REGRA DURA: a liberação por confiança NÃO dá baixa na fatura — NUNCA diga que o pagamento foi baixado/quitado nem que a fatura está paga; a fatura segue EM ABERTO até o financeiro confirmar.
...
async function conferirValorComprovante(
  ...
  // PIX do Asaas dá BAIXA AUTOMÁTICA no SGP (caso Rosevaldo, 10/08/2026): o
  // cliente paga, a fatura some da lista e ele ainda manda o comprovante. Se
  // NADA está vencido (a mais antiga em aberto vence no futuro), o pagamento
  // já foi processado — comparar com a fatura do MÊS QUE VEM e "liberar por
  // confiança" um cliente que nem bloqueado está só confunde.
  const hoje = new Date().toLocaleDateString("en-CA", { timeZone: "America/Bahia" }); // YYYY-MM-DD
  const vencidaMaisAntiga = titulos.find((t) => String(t.vencimento ?? "").slice(0, 10) <= hoje);
  if (!titulos.length) {
    args.valor_confere = true;
    args.ja_baixado = true;
    return "Conferência automática do sistema: NENHUMA fatura em aberto — o pagamento já foi BAIXADO automaticamente (PIX/Asaas).";
  }
  if (titulos.length && !vencidaMaisAntiga) {
    args.valor_confere = true;
    args.ja_baixado = true;
    return (
      `Conferência automática do sistema: NENHUMA fatura vencida em aberto (a mais antiga vence ${String(titulos[0]?.vencimento ?? "").slice(0, 10)}). ` +
      `O pagamento provavelmente já foi BAIXADO automaticamente (PIX/Asaas).`
    );
  }
  const devido = vencidaMaisAntiga?.valor ?? titulos[0]?.valor; // mais antiga em aberto (já ordenada)
...
async function executeTool(name: string, args: Record<string, unknown>, sgpList: { id: string; client: SgpClient; pixChaveManual?: string }[], defaultSgpId: string | undefined, memo: SgpMemo): Promise<unknown> {
  ...
  if (name === "registrar_comprovante") {
    ...
    return {
      ...
      instrucao:
        args.ja_baixado === true
          ? "O pagamento JÁ FOI BAIXADO automaticamente no sistema (nada vencido em aberto). NÃO chame liberacao_confianca e NÃO fale de liberação, bloqueio ou acesso. Responda apenas: agradeça o comprovante e confirme que o pagamento já está registrado e está tudo certo. Transfira ao financeiro só como registro, com motivo 'comprovante — pagamento já baixado automaticamente, nada a fazer'."
          : vc === true
  ...
  try {
    switch (name) {
      case "consultar_cliente": {
        ...
        return {
          ...
          contratos: c.contratos.map((ct) => ({
            ...
            status: ct.status,
            plano: ct.plano,
            // Nome do campo importa: o SGP só soma aqui o VENCIDO — chamar de
            // "valorEmAberto" fazia o modelo responder "não possui faturas"
            // p/ fatura vencendo HOJE sem nunca chamar faturas_em_aberto.
            valorVencido: ct.valorEmAberto,
            endereco: ct.endereco,
          })),
          aviso: "valorVencido soma APENAS faturas já vencidas. Para saber se há faturas a pagar (inclusive vencendo hoje ou futuras), chame faturas_em_aberto.",
        };
      }
      ...
      case "liberacao_confianca": {
        ...
        if (!r.ok) {
          // Falha de liberação é assunto INTERNO (pedido do dono, 11/08): o
          // cliente recebe só o encaminhamento — nada de "tentei liberar",
          // "acesso" ou "não foi possível" (cliente: "ACESSO DE QUE?").
          return {
            ok: false,
